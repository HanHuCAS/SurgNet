...
class MaskedAutoencoderPVT(nn.Module):
    ...
    def denormal(self,img):
        ...
        g = g*std[1] + mean[1]
        b = b*std[2] + mean[2]
        img = torch.cat([r,g,b],1)
        return img
        
    def forward_test(self, imgs, mask, iter_id):
        """latent = self.forward_encoder(imgs, mask) # returned mask may change
        pred, mask_num = self.forward_decoder(latent, mask)  # [N, L, p*p*3]
        target = self.patchify(imgs, self.patch_size)
        pred = self.patchify(self.unpatchify(pred, self.patch_size*4), self.patch_size)
        mean = target.mean(dim=-1, keepdim=True)
        var = target.var(dim=-1, keepdim=True)
        pred = (pred * (var**.5) + mean).clamp(imgs.min(), imgs.max())
        
        mask_ = mask.unsqueeze(-1)
        print(mask_.size())
        print(mask.size())
        print(imgs.size())
        masked_input_output = self.unpatchify((~mask_)*self.patchify(imgs, self.patch_size), self.patch_size)
        save_image(masked_input_output, './show/' + str(iter_id) + "masked_input.jpg")
        save_image(self.unpatchify((~mask_)*self.patchify(imgs, self.patch_size)+mask_*pred, self.patch_size), './show/' + str(iter_id) + "recon.jpg")
        save_image(self.unpatchify(pred, self.patch_size), './show/' + str(iter_id) + "pred.jpg")
        save_image(imgs, './show/' + str(iter_id) + "input.jpg"）"""      
        """latent = self.forward_encoder(imgs, mask) # returned mask may change
        pred, mask_num = self.forward_decoder(latent, mask)  # [N, L, p*p*3]
        target = self.patchify(imgs, self.patch_size)
        pred = self.patchify(self.unpatchify(pred, self.patch_size*4), self.patch_size)
        ...
        var = target.var(dim=-1, keepdim=True)
        pred = (pred * (var**.5) + mean).clamp(imgs.min(), imgs.max())
        pred_img = self.denormal(self.unpatchify(pred, self.patch_size))
        mask_ = mask.unsqueeze(-1)
        masked_input_output = self.denormal(self.unpatchify((~mask_)*self.patchify(imgs, self.stride), self.stride))
        save_image(pred_img, './show/' + str(iter_id) + "_pred.jpg")
        save_image(self.denormal(imgs), './show/' + str(iter_id) + "_ori.jpg")
        save_image(masked_input_output, './show/' + str(iter_id) + "masked_input.jpg")"""
        latent = self.forward_encoder(imgs, mask) # returned mask may change
        pred, mask_num = self.forward_decoder(latent, mask)  # [N, L, p*p*3]
        target = self.patchify(imgs, self.stride)
        pred = self.patchify(self.unpatchify(pred, self.stride), self.stride)
        mean = target.mean(dim=-1, keepdim=True)
        var = target.var(dim=-1, keepdim=True)
        pred = (pred * (var**.5) + mean)#.clamp(imgs.min(), imgs.max())
        pred_img = self.denormal(self.unpatchify(pred, self.stride))
        mask_ = mask.unsqueeze(-1)
        masked_input_output = self.denormal(self.unpatchify((~mask_)*self.patchify(imgs, self.stride), self.stride))
        save_image(pred_img, './show/' + str(iter_id) + "_pred.jpg")
        save_image(self.denormal(imgs), './show/' + str(iter_id) + "_ori.jpg")
