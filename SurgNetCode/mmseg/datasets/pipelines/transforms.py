...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        #print(img.shape)
        #cv2.imwrite('/home/jcchen/SegFormer/work_dirs/'+str(img.shape)+'.png',img)
...
class Elasticdeform(object):
    def __init__(self, radom_elast_ratio = 1):
        self.radom_elast_ratio = radom_elast_ratio
    
    def __call__(self, results):
        
        [results['img'], results['gt_semantic_seg']] = elasticdeform.deform_random_grid([results['img'].astype('float64'), results['gt_semantic_seg']], sigma=25, points=3, axis=(0 ,1), order=[3, 0])
        [results['img'], results['gt_semantic_seg']] = elasticdeform.deform_random_grid([results['img'], results['gt_semantic_seg']], sigma=5, points=18, axis=(0 ,1), order=[3, 0])
        [results['img'], results['gt_semantic_seg']] = elasticdeform.deform_random_grid([results['img'], results['gt_semantic_seg']], sigma=15, points=9, axis=(0 ,1), order=[3, 0])
        results['img'] = results['img'].clip(0, 255).astype('uint8')
        return results
